...
# Set up module paths
modules_path_1 = os.path.join(os.getcwd(), './Federation4')
...
def main():
    ...
    verify_process = subprocess.Popen(
        ["python", "verify.py"],
        cwd="../Communities/community4_endpoint",
    ...
    for i in range(20):  # Run the sequence 20 times
        ...
        interaction_id, pid = create_Interaction(
            "Federation4", "Community1", "Community4", "community", "active",
            "Brick", "Brick", "community1/occupancy", "community4/occupancy"
        )
    ...
    if os.path.exists("startup_log_brick_brick.txt"):
        new_file_path = os.path.join(results_dir, "startup_log_brick_brick.txt")
        shutil.move("startup_log_brick_brick.txt", new_file_path)
        print(f"📂 File moved successfully to {new_file_path}.")
    else:
        print("⚠️ startup_log_brick_brick.txt not found; skipping move.")
