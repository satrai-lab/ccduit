...
def monitor_Interaction(interaction_id, Federation_ID, existing_policy, pid):
    ...
    def on_message(client, userdata, message):
        ...
        if new_policy_data["modificationPolicy"]["value"]["lastModified"] != existing_policy["modificationPolicy"]["value"]["lastModified"]:
            ...
            if not canReceive:
                policy_update_time = int(new_policy_data.get("timestamp", 0))  # Default to 0 if not found
                # print(policy_update_time)
                print(f"[on_message] Terminating interaction {interaction_id} due to policy restrictions.")
                terminate_Interaction(interaction_id=interaction_id, pidInput=pid,policy_update_time=policy_update_time)
...
def terminate_Interaction(interaction_id,pidInput=None,policy_update_time=0):
