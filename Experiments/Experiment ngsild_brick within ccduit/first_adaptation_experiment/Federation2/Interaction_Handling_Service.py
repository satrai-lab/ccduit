...
def http_worker(community_context_url, message_queue):
    while True:
        try:
            ...
            try:
                # Measure delay since message was queued
                # queued_time_ns = data["timestamp_ns"]
                # time_now_ns = time.perf_counter_ns()
                # delay_ns = time_now_ns - queued_time_ns
                # print(f"Message waited in queue for {delay_ns / 1_000_000} ms before posting.")

                # Send the data via HTTP
                # before_posting= time.perf_counter_ns()
                ...
                if response.status_code in [200, 201]:
                    delay_ms = (time.perf_counter_ns() - data["timestamp_ns"]) / 1_000_000
...
def interaction_process(interaction_id, source_community, destination_community, Interaction_Type,
    ...
    if source_protocol.lower()=="mqtt" and destination_protocol.lower() == "http":  
        ...
        p = multiprocessing.Process(target=http_worker, args=(community_context_url,message_queue),daemon=True)
        p.start()
...
def monitor_Interaction(interaction_id, Federation_ID, existing_policy, pid):
    ...
    def on_message(client, userdata, message):
        ...
        # print(f"[on_message] Message received on topic {message.topic}")
