...
def main():
    ...
    plt.bar(df['Run'], df['Delay'], color='skyblue', edgecolor='black', label='responsive Delay')
    ...
    plt.xlabel('Run')
    plt.ylabel('Delay (ms)')
    plt.title('Policy Adaptation Delay for 100 Runs with Overall Average')
    plt.xticks(range(1, 101))  # Ensure x-axis shows runs 1 through 20
    plt.legend()
