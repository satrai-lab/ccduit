...
def main():
    ...
    if os.path.exists("startup_log_ngsild_brick.txt"):
        new_file_path = os.path.join(results_dir, "startup_log_ngsild_brick.txt")
        shutil.move("startup_log_ngsild_brick.txt", new_file_path)
        print(f"📂 File moved successfully to {new_file_path}.")
    else:
        print("⚠️ startup_log_ngsild_brick.txt not found; skipping move.")
