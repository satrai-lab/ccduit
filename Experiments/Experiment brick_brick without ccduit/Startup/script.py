...
def run_occupancy_script():
    ...
    print("Running community1_Occupancy.py...")
    occupancy_process = subprocess.Popen('start cmd /k python community1_Occupancy.py', shell=True, cwd="./")
    # occupancy_process = subprocess.Popen(
    #     ["python", "community1_Occupancy.py"],
    #     cwd="./"
    # )
...
def main():
    ...
    for i in range(20):  # Run the sequence 20 times
        ...
        time.sleep(2)
        print(f"🟢 Run {i + 1} - Completed.")
        # Let all processes run concurrently for a fixed period (e.g., 10 seconds)
        # time.sleep(10)
